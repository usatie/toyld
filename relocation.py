...
def _relocate_a4(rel, obj, gsymtab, seg_data, offset):
...
def _relocate_r4(rel, obj, gsymtab, seg_data, offset, tgt_lseg):
    ref_lseg = obj.segments[rel.ref - 1]
...
def _relocate_rs4(rel, obj, gsymtab, seg_data, offset, tgt_lseg):
    ...
    ref_sym = gsymtab[sym_name]
    addend = int.from_bytes(seg_data[offset:offset+4], byteorder='big', signed=True)
    pc_relative_addr = ref_sym.value + addend - (tgt_lseg.assigned_address + rel.loc + 4)
    # Big-endian 4-byte absolute symbol address copy
...
def _relocate_one(rel, obj, gsymtab, gdata):
    ...
    if offset + 4 > len(seg_data):
        ...
        sys.exit(1)
    if rel.rel_type == 'A4':
        return _relocate_a4(rel, obj, gsymtab, seg_data, offset)
    elif rel.rel_type == 'R4':
        return _relocate_r4(rel, obj, gsymtab, seg_data, offset, tgt_lseg)
